...
//adds message to the correct array flag 
...
// JSON stores the messages
 ...
 if (file.exists()) { 
 String content = new String(Files.readAllBytes(file.toPath())).trim();  if (content.length() > 2) { // more than just "[]" 
 content = content.substring(1, content.length() - 1).trim();  if (!content.isEmpty()) { 
 // split top-level objects (simple split on "},{")  String[] parts = content.split("\\},\\s*\\{");  for (int i = 0; i < parts.length; i++) { 
 String part = parts[i]; 
 if (!part.startsWith("{")) part = "{" + part;  if (!part.endsWith("}")) part = part + "}";  existingEntries.add(part); 
 } 
 } 
 } 
 ...
 String newEntry = "{" 
 + "\"messageId\":\"" + escapeJson(message.getMessageId()) + "\","  + "\"messageHash\":\"" + escapeJson(message.getMessageHash())
         + "\","  + "\"recipient\":\""
         + escapeJson(message.getRecipient()) 
         + "\","  + "\"message\":\"" 
         + escapeJson(message.getMessageText()) + "\""  + "}"; 
 ...
 try { 
 String content = new String(Files.readAllBytes(file.toPath())).trim();  if (content.length() <= 2) return result; // empty array "[]" 
 content = content.substring(1, content.length() - 1).trim();  String[] entries = content.split("\\},\\s*\\{"); 
 for (String entry : entries) { 
 if (!entry.startsWith("{")) entry = "{" + entry; 
 if (!entry.endsWith("}")) entry = entry + "}"; 
 String recipient = extractJsonValue(entry, "recipient");  String text = extractJsonValue(entry, "message"); 
...
public String findLongestMessage() { 
 ...
 for (Message m : all) { 
 if (longest == null || m.getMessageText().length() > longest.getMessageText().le longest = m; 
 } 
 } 
...
public String searchByMessageId(String idToFind) { 
 ...
 all.addAll(storedMessages); 
 for (Message m : all) { 
 if (idToFind.equals(m.getRecipient())) { 
 return m.getMessageText(); 
 } 
 } 
 return "Message ID not found."; 
 } 
// returns messagesz for the given recipient 
...
//deletes a message sent , stored or displayed 
 
 List<List<Message>> allArrays = List.of(sentMessages, storedMessages, disregardedMes
...
// returns stored messages while the program is running 
